...
利用者が良いと感じた録音(My_Recording_3)の要点は、音域の広さではなく「最初から最後まで口ずさめること」だった。
録音の冒頭は B♭→E♭→B♭ と5度を連ね、全体ではオクターブを超える。それでも口ずさめる。

以前の核の歌いやすさ(`judgeCoreMotif`)は、音域が10半音を超えると点を下げ、跳躍が2つ以上あると大きく下げていた。
そのため録音の冒頭は 0.48 と、録音の中でいちばん「歌いにくい」と判定されていた。いまは次の3つで見る(録音の2小節ごとの平均 0.80 → 0.93、最低 0.48 → 0.75。既知の反例に対して物差しが適切になったという結果で、生成した旋律の質が上がったという意味ではない。ほかの正例・負例での確認はまだ)。

- 音域の広さの代わりに、高さが変わる速さ(同じ音の刻みは速くても歌える)
- 順次進行の割合の代わりに、音程ごとの歌いやすさ(同音・2度・3度・4度・5度・オクターブは歌える。核の中では順次をわずかに優先する)
- 跳躍の数の代わりに、歌いにくい音程(増4度・7度・オクターブを超える跳躍)の数

旋律全体の口ずさみやすさ(`measureHummability`、いちばん歌いにくい2小節を重く見る)も測った。
いまの全ての作り方で 0.87〜0.98(録音 0.94)と差が小さく、候補の選抜に入れても選ばれる旋律はほとんど変わらなかったので、選抜には使わず検証だけに使う。
