...
| Brahms | 動機を保った発展 | develop（リズムと輪郭を保ち、1か所の音程だけ広げる）。8 小節で使う。16 小節以上では前半に早すぎる頂点を作ったため使わない |
...
## 余白と頂点の時機の下限（Boutonnat フィルタ）

- 頂点の前後どちらにも息継ぎが無いときは、頂点の直前の音を少し短くして 0.5 拍の間を作る（`ensureSummitBreath`、音程と発音位置は変えない）。
- 16 小節以上では、頭を戻すときに頂点の目標位置に近いフレーズの高い音を下げない（頂点が前へずれないように）。
- 頂点が早すぎ（サビなら 57% より前）、後半にほぼ同じ高さ（2 半音以内）の音があるときだけ、早い頂点を和音の構成音へ少し下げて後半に頂点を譲る（`deferEarlySummit`）。最初のフレーズ（核）には触らない。
