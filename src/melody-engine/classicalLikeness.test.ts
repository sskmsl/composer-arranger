...
import { buildFeatureModel, classicalLikeness, densityAt, type ClassicalModel } from "./classicalLikeness"
...
describe("古典らしさ(古典=100)", () => {
  ...
  it("跳び回る旋律は古典らしさが低く、内訳に古典の中央値が付く", () => {
    const result = classicalLikeness(measureMelodyCraft(leaping, chords, "C"), model)
  ...
  it("推敲すると古典らしさが上がり、形(音の数・リズム)と冒頭・最後の音・固定した音は変えない", () => {
    ...
    const result = refineTowardClassical(locked, {
      harmonicMap: buildHarmonicMap(chords), range: { low: 55, high: 81 }, totalBeats: 32, sectionRole: "verse", key: "C", model,
    })
