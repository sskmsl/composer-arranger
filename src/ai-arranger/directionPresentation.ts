...
export function plainDirectionText(value: string): string {
  return value
    ...
    // 内部の設計用語(英語混じり)を、画面で読める言葉へ
    .replace(/Set\s*Active\s*(?:し|して)?/gi, "採用し")
    .replace(/Active\s*(?:素材|構成)/gi, "採用中の素材")
    .replace(/Active\s*(?:Layer|レイヤー)/gi, "鳴っている追加パート")
    .replace(/高\s*Energy\s*(?:側|Section|セクション)?/gi, "盛り上がる所")
    .replace(/低\s*Energy\s*(?:側|Section|セクション)?/gi, "静かな所")
    .replace(/\bEnergy\b/gi, "盛り上がり")
    .replace(/\bMotif\b/gi, "短い音型")
    .replace(/\bRoles?\b/gi, "役割")
    .replace(/\bLayers?\b/gi, "レイヤー")
    .replace(/\bVelocity\b/gi, "音の強さ")
    .replace(/\bDynamic\b/gi, "強弱")
    .replace(/\bPreview\b/gi, "試聴")
    .replace(/\bwithdraw\b/gi, "退場")
