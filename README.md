...
| 採用結果の MIDI 書き出し、役割に沿ったVelocity・Gate・Accent・Timing | 音源固有の奏法調整・ミックス・マスタリング |
...
- **MIDI Export**: セクション単位・曲全体単位で SMF 書き出し。セクション名をマーカーとして出力し、Logic Pro 側で構成を確認できる。Counter/Decorationは専用トラックとして分離。新規候補にはパートとSectionの役割に応じたVelocity・Gate・Accent・Timingを適用
